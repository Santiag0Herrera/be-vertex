...
class DocumentRequest(BaseModel):
  ...
  emisor_cuit: str = Field(..., pattern=r"^(\d{8}|\d{11})$", description="CUIT of the sender, must be 11 digits")
  emisor_cbu: Optional[str] = Field(None, pattern=r"^\d{22}$")
  receptor_name: str = Field(..., min_length=1, description="Name of the receiver")
  receptor_cuit: str = Field(..., pattern=r"^(\d{8}|\d{11})$", description="CUIT of the receiver, must be 11 digits")
